...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!url && !raw_content) {
      return new Response(JSON.stringify({ error: 'URL or text content is required' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (providers && providers.length > 0) {
      const systemPrompt = `Você é um especialista em tecnologia audiovisual analisando novidades e informações de mercado.
  ...
  "summary": "..."
      ...
      for (const provider of providers) {
        ...
        try {
          ...
          try {
            responseText = await callAIProvider(pType, customEndpoint, modelId, apiKey, systemPrompt, truncatedContent, controller.signal)
          } finally {
