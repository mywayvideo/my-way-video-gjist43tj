...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const systemPromptTemplate = aiSettings?.system_prompt_template || "Você é um assistente sênior."
...
- Mantenha os parágrafos curtos (máximo 2 sentenças por parágrafo).
- Use blocos de código ( \`\`\` ) para formatar especificações técnicas.
- NUNCA mencione IDs de produtos.
- SEMPRE inclua no final o aviso de que todos os produtos contam com garantia no Brasil e América Latina (LATAM), com envio imediato de Miami.
- Regras Logísticas para origem de envio: ${logisticsRulesPrompt}
${hasIntelData || isNABQuery ? '- É EXPRESSAMENTE PROIBIDO dizer que "não há informações". Utilize os dados reais da inteligência fornecida.' : ''}
