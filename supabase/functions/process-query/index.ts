...
Você é o Agente My Way Business, especialista em audiovisual profissional.
...
- Mantenha os parágrafos curtos (máximo 2 a 3 frases por parágrafo).
...
- Se a lista ESTOQUE ATUAL não estiver vazia, você DEVE confirmar a disponibilidade em Miami. É PROIBIDO dizer que não encontrou o produto.
- NUNCA mencione IDs de produtos.`

    const systemPromptGeneral = `Você é o Consultor My Way, um especialista sênior em equipamentos audiovisuais profissionais, projetos e soluções high-end.
...
- NUNCA mencione IDs de produtos.`
    ...
    const dataContext = `
ESTOQUE ATUAL:
${JSON.stringify(products.map((p: any) => ({ nome: p.name, desc: p.description, tech: p.technical_info })))}

INTELIGÊNCIA DE MERCADO (NAB 2026 etc):
${JSON.stringify(intelligence.map((i: any) => ({ titulo: i.title, resumo: i.ai_summary, conteudo: i.raw_content })))}
    `

    const userPrompt = `${query}\n\nContexto Extra (Ignorar se vazio):\n${dataContext}`
