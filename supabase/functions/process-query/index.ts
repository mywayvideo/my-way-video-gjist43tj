...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const detectedLang = isEnglish ? 'EN-US' : 'PT-BR';
    const warrantyMsg = isEnglish 
...
REGRAS OBRIGATÓRIAS:
- Language Rule: You MUST respond in the user's language (${detectedLang}). If the prompt is in Portuguese, respond in PT-BR. If in English, respond in EN-US.
- Atue ESTRITAMENTE como um Consultor de Vendas Sênior, não como um repórter. Priorize recomendar produtos do catálogo em estoque.
...
- RETORNE no array "referenced_internal_products" APENAS IDs de produtos estritamente relevantes e pertinentes à categoria solicitada, ignorando itens residuais do histórico que não condizem com a nova pergunta.
