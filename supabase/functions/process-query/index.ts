...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const systemPromptTemplate = aiSettings?.system_prompt_template || "Você é o Especialista My Way Business. Sua prioridade é o hardware de maior valor (price_usd) retornado pela busca. É PROIBIDO omitir cards de produtos mencionados no texto. Siga as instruções da Section G para logística."
    ...
    const isEnglish = /^(what|how|why|where|when|can|do|is|are|will|would|could|should|please|show|tell)\b/i.test(qLower) || 
                      /\b(price|camera|lens|shipping|buy|purchase|warranty)\b/i.test(qLower);
    const warrantyMsg = "Todos os serviços e produtos da My Way estão cobertos pela nossa garantia oficial Brasil/LATAM.";

    const dynamicConstraints = `
REGRAS OBRIGATÓRIAS:
- TODAS AS RESPOSTAS DEVEM SER EM PORTUGUÊS (PT-BR), independentemente do idioma da pergunta.
- Atue ESTRITAMENTE como um Consultor de Vendas Sênior. Priorize o hardware de maior valor (price_usd) retornado pela busca.
- É PROIBIDO fazer comparações não solicitadas a menos que o usuário pergunte "Qual a melhor?" ou "Compare".
...
- SEMPRE inclua o aviso de garantia oficial ao final da resposta: "${warrantyMsg}"
- NUNCA mencione IDs de produtos no texto.
- É ESTRITAMENTE PROIBIDO omitir cards de produtos mencionados no texto.
- RETORNE no array "referenced_internal_products" APENAS IDs de produtos relevantes e mencionados, garantindo que o produto principal perguntado pelo usuário seja incluído.
${hasIntelData && !isNABQuery ? '- Mencione informações de eventos APENAS como selo de qualidade para os produtos recomendados, nunca como assunto principal.' : ''}
${isNABQuery ? '- Como o cliente perguntou especificamente sobre a feira, traga os dados da NAB 2026 e inicie com: "Confirmamos diretamente da NAB 2026: "' : ''}
`
