...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { query, products = [], intelligence = [], agentId } = await req.json()
    ...
    const systemPrompt = 'Você é o Agente My Way Business, especialista em audiovisual profissional. Use os dados fornecidos para responder a pergunta. Se os dados forem da NAB 2026 ou similares, inclua a frase: "Confirmamos diretamente da NAB 2026: " se apropriado. Responda APENAS em PT-BR. Mantenha parágrafos curtos (máximo 2 a 3 frases). OBRIGATÓRIO: Use blocos de código (```) para formatar quaisquer especificações técnicas.'
...
${JSON.stringify(intelligence.map((i: any) => ({ titulo: i.title, resumo: i.ai_summary, conteudo: i.raw_content })))}
    ...
    const userPrompt = `Contexto de Dados:\n${dataContext}\n\nPergunta do usuário: ${query}`
