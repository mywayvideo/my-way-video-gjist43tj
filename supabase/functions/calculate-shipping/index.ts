...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: "Dados invalidos. Verifique os campos obrigatorios: tipo_entrega, endereco, itens_carrinho." }), { 
        status: 400, 
    ...
    if (delivery_type !== 'coleta' && (!address.street || !address.city || !address.state || !address.zip_code)) {
      return new Response(JSON.stringify({ error: "Endereco incompleto. Preencha: rua, cidade, estado, CEP/ZIP." }), { 
        status: 400, 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      });
    }
    ...
    if (delivery_type === 'coleta') {
      ...
      const warehouseStr = settingsData?.find(s => s.setting_key === 'warehouse_location')?.setting_value;
      const rangesStr = settingsData?.find(s => s.setting_key === 'shipping_miami_ranges')?.setting_value;
      ...
      try {
        ...
        if (destLat === 0 && destLng === 0) {
          ...
          const numberPart = (address.number && address.number !== '0' && address.number.toLowerCase() !== 's/n') ? ` ${address.number}` : '';
        ...
        return new Response(JSON.stringify({ error: "Endereco nao encontrado. Verifique o CEP/ZIP e tente novamente." }), { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        });
      }

      if (typeof destLat !== 'number' || typeof destLng !== 'number' || isNaN(destLat) || isNaN(destLng) || (destLat === 0 && destLng === 0)) {
        return new Response(JSON.stringify({ error: "Nao foi possivel calcular a distancia. Verifique o endereco." }), { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        });
      }
      ...
      if (!matchedRange) {
        return new Response(JSON.stringify({ error: "Distancia alem do perimetro maximo de Miami. Selecione 'Entrega EUA' para usar UPS." }), { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        });
      }
      ...
      try {
        ...
        return new Response(JSON.stringify({ error: "Erro ao processar frete. Tente novamente." }), { 
      ...
      try {
        ...
        const priceData = settingsData?.find(s => s.setting_key === 'shipping_sao_paulo_price_per_kg');
        if (priceData) price_per_kg = priceData.setting_value_numeric ?? Number(priceData.setting_value);

        const percData = settingsData?.find(s => s.setting_key === 'shipping_sao_paulo_percentage_value');
        if (percData) percentage_value = percData.setting_value_numeric ?? Number(percData.setting_value);

        const addWeightData = settingsData?.find(s => s.setting_key === 'shipping_sao_paulo_additional_weight_kg');
        if (addWeightData) {
          const val = addWeightData.setting_value_numeric ?? Number(addWeightData.setting_value);
          if (!isNaN(val)) additional_weight_kg = val;
        }

        if (price_per_kg === null || percentage_value === null || isNaN(price_per_kg) || isNaN(percentage_value)) {
          return new Response(JSON.stringify({ error: "Configuracao de frete incompleta. Verifique as configuracoes no painel admin." }), { 
            status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
          });
        }
      } catch (e: any) {
      ...
      try {
        for (let i = 0; i < cart_items.length; i++) {
          ...
          const lb = Number(item.weight_lb !== undefined ? item.weight_lb : (item.weight !== undefined ? item.weight : item.weight_kg));
        ...
        return new Response(JSON.stringify({ error: "Nao foi possivel calcular o frete. Tente novamente." }), { 
          status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        });
      }
      ...
      for (const item of cart_items) {
        let lb = Number(item.weight_lb !== undefined ? item.weight_lb : (item.weight !== undefined ? item.weight : item.weight_kg));
      ...
      try {
        ...
        console.log("[calculate-shipping] Invoking ups-calculate-rate with payload:", JSON.stringify(upsPayload));

        const { data: upsData, error: upsError } = await supabase.functions.invoke('ups-calculate-rate', {
          body: upsPayload
        });

        console.log("[calculate-shipping] ups-calculate-rate response:", JSON.stringify({ upsData, upsError }));
      ...
      if (!upsSuccess) {
        console.log("[calculate-shipping] UPS calculation failed or returned no services. Using fallback formula.");
        ...
        try {
          ...
          if (formulaData?.setting_value) {
            ...
            const baseCost = (totalValue * (Number(formula.value_percentage) || 0) / 100) + (totalWeightKg * (Number(formula.weight_price_per_kg) || 0)) + (Number(formula.base_cost) || 0);
            cost = Math.ceil(baseCost * 10) / 10;
          } else {
            console.error("[calculate-shipping] Fallback formula not configured in app_settings (shipping_usa_formula)");
            throw new Error("Sem formula configurada");
          }
        } catch (e: any) {
          console.error("[calculate-shipping] Error using fallback formula:", e.message);
          return new Response(JSON.stringify({ error: "Não foi possível calcular frete (UPS e fallback falharam). Verifique as configurações de frete." }), { 
            status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
          });
        }
      }
    ...
    return new Response(JSON.stringify({ error: "Erro interno no servidor ao processar o frete." }), { 
      status: 500, 
      headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
    });
  }
