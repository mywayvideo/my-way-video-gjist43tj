...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (delivery_type === 'coleta') {
      ...
      const cost = (totalValue * formula.value_percentage / 100) + (totalWeight * formula.weight_price_per_kg);
      ...
      const cost = (totalValue * (formula.value_percentage || 0) / 100) + (totalWeight * (formula.weight_price_per_kg || 0)) + (formula.base_cost || 0);

      return new Response(JSON.stringify({
        shipping_cost: cost,
        message: `Frete EUA: peso ${totalWeight.toFixed(2)} kg, frete USD ${cost.toFixed(2)}`,
        delivery_type
      }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    } else {
