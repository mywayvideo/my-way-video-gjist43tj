  ...
  try {
    ...
    if (delivery_type === 'coleta') {
      ...
      try {
        ...
        const formulaStr = settingsData?.find(s => s.setting_key === 'shipping_sao_paulo_formula')?.setting_value;
        const addWeightStr = settingsData?.find(s => s.setting_key === 'shipping_sao_paulo_additional_weight')?.setting_value;
      ...
      try {
        ...
        const baseCost = (totalValue * (Number(formula.value_percentage) || 0) / 100) + (finalWeightKg * (Number(formula.weight_price_per_kg) || 0));
        cost = Math.ceil(baseCost * 10) / 10;
      } catch(e) {
        return new Response(JSON.stringify({ error: "Erro ao processar frete. Tente novamente." }), { 
          status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
