...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      if (cacheData && cacheData.cached_at) {
        ...
        if (now - cachedAt < sevenDaysMs) {
          const specs = typeof cacheData.product_specs === 'object' && cacheData.product_specs !== null 
    ...
    let systemPrompt = "You are a senior expert in professional audiovisual products. Extract ALL detailed technical specifications from the product. Provide information a professional needs. Include compatibility, use cases, and related accessories."
    let aiModelsOrder = ['deepseek', 'gpt-4o-mini', 'claude-3-5-sonnet']
    let aiModelsActive: Record<string, boolean> = { 'deepseek': true, 'gpt-4o-mini': true, 'claude-3-5-sonnet': true }
      ...
      while (attempt < maxAttempts && !success) {
        try {
          ...
          try {
            ...
            else if (lowerName.includes('gpt-4o-mini') || lowerName.includes('gpt4')) finalConfidence = 'medium';
            else if (lowerName.includes('claude') || lowerName.includes('sonnet')) finalConfidence = 'low';
          ...
          const isNetworkError = error?.name === 'AbortError' || error?.message?.toLowerCase().includes('fetch') || error?.message?.toLowerCase().includes('network');
          attempt++;
    ...
    try {
      ...
      const finalSpecs = typeof enrichedData === 'object' && enrichedData !== null 
        ? { ...enrichedData, _confidence_level: finalConfidence, _ai_model_used: successfulModel }
        : { data: enrichedData, _confidence_level: finalConfidence, _ai_model_used: successfulModel };
