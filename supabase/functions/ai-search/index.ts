...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (aiSettings?.custom_stop_words) {
      aiSettings.custom_stop_words.split(',').map((w: string) => w.trim().toLowerCase()).forEach((w: string) => stopWords.add(w));
    }
...
- É OBRIGATÓRIO retornar TODOS os IDs (ou SKUs) exatos dos produtos do Inventário que você mencionou na resposta no array "referenced_internal_products". Isso é crítico para exibir as imagens na tela.
