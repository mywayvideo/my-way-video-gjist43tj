...
- For EVERY product mentioned, you MUST create a dedicated section titled '### Especificações Base' immediately after the description.
          ...
          if (msg?.tool_calls) {
            ...
            if (calls === 0) {
              ...
              if (hasSemanticViolations || providedIds.length > 0) {
                msgs.push(msg)
                msgs.push({ role: 'system', content: "SECURITY VIOLATION: You are attempting to respond using internal knowledge. You are FORBIDDEN from answering without calling 'search_products'. Execute the tool now to fetch real UUIDs and stock data." })
                calls++
                continue
              }
            }
