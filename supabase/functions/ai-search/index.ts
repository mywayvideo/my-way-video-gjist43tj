            ...
            for (const t of msg.tool_calls) {
              if (t.function.name === 'search_products') {
                ...
                msgs.push({
                  role: 'system',
                  content:
                    'Inventory received (Top 10 matches). Synthesize a concise response. If the user asked for a specific series (like Pocket), focus ONLY on those. Return the JSON object immediately.',
                })
              }
            }
            calls++
