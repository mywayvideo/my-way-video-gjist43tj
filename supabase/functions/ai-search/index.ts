...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Prioritize terms with numbers (like 7m4)
    const alphaNumTerms = qTerms.filter((t: string) => /[0-9]/.test(t));
    if (alphaNumTerms.length > 0) {
      qTerms = alphaNumTerms;
    } else if (qTerms.length === 0 && allTerms.length > 0) {
      qTerms = allTerms;
    }
