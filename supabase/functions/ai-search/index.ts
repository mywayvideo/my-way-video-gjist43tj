          ...
          if (msg?.tool_calls) {
            ...
            for (const t of msg.tool_calls) {
              if (t.function.name === 'search_products') {
                ...
                const top10Stock = (rpcData?.stock || []).slice(0, 10);
                top10Stock.forEach((p: any) => { if (p.id) allowedProductIds.add(p.id) });

                let content = JSON.stringify({
                   stock: top10Stock.map((prod: any) => ({
                      id: prod.id, name: prod.name, sku: prod.sku, price_usd: prod.price_usd, manufacturer_name: prod.manufacturer_name
                   })),
                   intel: (rpcData?.intel || []).slice(0, 3),
                   nab_data: (rpcData?.nab_data || []).slice(0, 3)
                });
                
                msgs.push({ role: 'tool', tool_call_id: t.id, name: t.function.name, content })
                msgs.push({ role: 'system', content: "Inventory received (Top 10 matches). Synthesize a concise response. If the user asked for a specific series (like Pocket), focus ONLY on those. Return the JSON object immediately." })
              }
            }
            calls++
          } else {
