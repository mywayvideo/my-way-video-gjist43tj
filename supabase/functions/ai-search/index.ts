...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('Env vars present:', { SUPABASE_URL: !!supUrl, SUPABASE_SERVICE_ROLE_KEY: !!serviceRoleKey });
    ...
    const stopWords = new Set(['que', 'qual', 'como', 'para', 'por', 'com', 'uma', 'um', 'tem', 'temos', 'voces', 'voce', 'mostrar', 'mostre', 'quero', 'gostaria', 'saber', 'preco', 'valor', 'sobre', 'esse', 'essa', 'este', 'esta', 'aqui', 'ali', 'cabo', 'the', 'what', 'who', 'how', 'why', 'can', 'you', 'show', 'tell', 'about', 'price', 'cost', 'favor', 'poderia', 'quais']);
    ...
    const orQuery = relevantTerms.length > 0 
      ? relevantTerms.map((t: string) => `title.ilike.%${t}%,raw_content.ilike.%${t}%,ai_summary.ilike.%${t}%`).join(',') 
    ...
    // Build the productOrQuery using the top 3 most specific (longest) terms found in the query.
    // This ensures "Burano" and "FX3" are both used if present.
    let productOrQuery = '';
    if (relevantTerms.length > 0) {
      const topTerms = [...relevantTerms].sort((a: string, b: string) => b.length - a.length).slice(0, 3);
      productOrQuery = topTerms.map((t: string) => 
    ...
    const { data: allProducts } = await supabase
      .from('products')
      .select(`id, name, sku, description, price_usd, price_cost, price_nationalized_sales, price_nationalized_cost, price_nationalized_currency, weight, category_id, manufacturer_id, technical_info, image_url, is_discontinued, stock, category`)
      .or(productOrQuery)
      .limit(50);
    ...
    const fallbackPrompt = "You are a Senior AV Solutions Expert. Your primary mission is to facilitate the sale by providing complete solutions.";
    ...
    if (settings.technical_bridge) {
      try {
        ...
        if (Array.isArray(bridges) && bridges.length > 0) {
          technicalBridgeRules = "\n\nREGRAS DE PONTE TÉCNICA (Obrigatório):\n" + bridges.map((b: any) => `- Se a fonte é ${b.source} e o destino é ${b.target}, você DEVE recomendar a solução: ${b.solution}`).join('\n');
        }
      } catch (e) {}
    }
...
MANDATORY RULES:
- You are a Senior AV Expert. Use 'intent_mapping' (Section H) and 'technical_bridge' (Section I) as your primary technical guides to expand context and solve gaps.
- However, you MUST always use the full Inventory to provide the best possible recommendations.
- If a user's request (e.g., 'clipes musicais', 'shows') matches a trigger in Section H, prioritize those expanded keywords.
- If it does NOT match a trigger, use your professional expertise to find the most suitable products from the Inventory.
- NEVER say you don't have information if there are relevant products in the Inventory list provided.
- If multiple valid solutions exist for a technical bridge (e.g., Teranex AND Optical Fiber), you MUST present and explain all of them, highlighting their differences in price and use case.
...
- Rule 3: If no direct replacement is found, state that you do not have complete information and offer to connect with a specialist.
- Rule 4: NEVER fabricate specifications or prices. Use USD pricing from the database.
- Rule 5: Mention that all products have manufacturer warranties in Brazil and Latin America.
- Rule 6: Paragraphs must be maximum 2 sentences. Technical specs in triple backtick code blocks.
- Rule 7: Start with a direct answer. Do NOT include product IDs in the text, only product names.
...
- É OBRIGATÓRIO retornar TODOS os IDs exatos dos produtos do Inventário que você recomendou na resposta no array "referenced_internal_products". Isso é crítico para exibir as imagens na tela.${technicalBridgeRules}${tonePrompt}
...
`
    ...
    for (const p of providers) {
      ...
      try {
        if (p.provider_name === 'gemini') {
          ...
          let msgs: any[] = [{ role: 'system', content: sysPrompt }, { role: 'user', content: actualQuery }];
    ...
    if (result.message) {
      ...
      if (normalizedMsg.includes('suporte') || normalizedMsg.includes('especialista') || normalizedMsg.includes('equipe')) {
          isLowConfidence = true;
      }
    ...
    let refs = Array.isArray(result.referenced_internal_products) ? result.referenced_internal_products : [];
