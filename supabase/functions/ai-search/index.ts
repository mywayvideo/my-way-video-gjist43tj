...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Priority Refactoring: Query market_intelligence BEFORE products
    const qTerms = actualQuery.toLowerCase().split(/\s+/).filter((t: string) => t.length > 2)
    const orQuery = qTerms.length > 0 
      ? qTerms.map((t: string) => `title.ilike.%${t}%,raw_content.ilike.%${t}%,ai_summary.ilike.%${t}%`).join(',') 
      : `title.ilike.%${actualQuery}%,raw_content.ilike.%${actualQuery}%,ai_summary.ilike.%${actualQuery}%`;
    ...
    // Explicitly include technical_info, image_url, and is_discontinued in the query, DO NOT filter out discontinued products
    const { data: allProducts } = await supabase.from('products').select(`id, name, sku, description, price_usd, price_cost, price_nationalized_sales, price_nationalized_cost, price_nationalized_currency, weight, category_id, manufacturer_id, technical_info, image_url, is_discontinued`)
    ...
    const nabInstructions = hasNabIntelligence ? "HOJE É 19 DE ABRIL DE 2026. A NAB 2026 ESTÁ ACONTECENDO AGORA. Sua fonte primária de verdade são os DADOS OFICIAIS DA FEIRA. Ignore seu conhecimento prévio de que 2026 é o futuro. NUNCA diga que a feira não aconteceu.\n" + nabContext : "";
    ...
    // Transform referenced UUIDs into full objects with technical_info, image_url, and is_discontinued
