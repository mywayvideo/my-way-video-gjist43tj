            ...
            for (const t of msg.tool_calls) {
              if (t.function.name === 'search_products') {
                ...
                try {
                  ...
                  if (!rpcData?.stock || rpcData.stock.length === 0) {
                    const queryStr = args.query || actualQuery;
                    const queryWords = queryStr.trim().split(/\s+/);
                    const lastWord = queryWords[queryWords.length - 1];
                    if (lastWord && lastWord.length > 2) {
                      console.log("TOOL CALL ARGS (Second Try):", lastWord);
                      const { data: secondData, error: secondError } = await supabase.rpc('execute_ai_search', { search_term: lastWord });
