...
- For every product discussed, you MUST create a section titled '### Especificações Base'.
- This section must include a bulleted list with: Resolução, Sensor, Codecs, Conectividade, Energia, Dimensões, and Peso.
- Include the field 'NCM: [value]' ONLY if the data exists in the database. If the NCM field is empty or null, do NOT display the NCM line.
        ...
        while (calls <= 2) {
          ...
          if (msg?.tool_calls) {
            ...
            if (calls === 0) {
              ...
              if (providedIds.length > 0 || hasInvalidIds) {
                msgs.push(msg)
                msgs.push({ role: 'system', content: "PROTOCOL ERROR: You provided technical data or IDs without consulting the real inventory. You are FORBIDDEN from inventing UUIDs. Call 'search_products' now to validate stock, prices, and NCM." })
                calls++
                continue
              }
            }
