...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const bypassCache = qL.includes('nab') || qL.includes('novidades') || qL.includes('feira') || qL.includes('2026');
    ...
    const qTerms = safeQueryForOr.toLowerCase().split(/\s+/).filter((t: string) => t.length > 2)
    const orQuery = qTerms.length > 0 
      ? qTerms.map((t: string) => `title.ilike.%${t}%,raw_content.ilike.%${t}%,ai_summary.ilike.%${t}%`).join(',') 
    ...
    const productOrQuery = qTerms.length > 0 
      ? qTerms.map((t: string) => `name.ilike.%${t}%,category.ilike.%${t}%,sku.ilike.%${t}%`).join(',')
...
REGRAS OBRIGATÓRIAS:
- Responda APENAS em Português (PT-BR). Atue como Consultor de Vendas Sênior, focando em vender o catálogo em estoque.
- Mantenha os parágrafos curtos (máximo 2 frases por parágrafo).
- Use blocos de código para formatar especificações técnicas.
- NUNCA mencione IDs de produtos nas mensagens.
- SEMPRE inclua o aviso de que todos os produtos contam com garantia no Brasil/LATAM e são enviados de Miami.
- Regras Logísticas: ${settings.logisticsRulesPrompt}
- Retorne em "referenced_internal_products" APENAS os IDs dos produtos que são ESTREITAMENTE RELEVANTES e pertencentes à MESMA CATEGORIA da busca. Exclua qualquer item de categoria irrelevante ou de contextos antigos. Se não houver itens precisos no contexto, retorne um array vazio.
${hasNabIntelligence ? '- Use os dados da feira APENAS como argumento de venda/selo de qualidade para os produtos do catálogo, não foque em notícias genéricas a menos que o usuário pergunte explicitamente sobre a NAB.' : ''}
...
`
    ...
    const sysPrompt = `${settings.systemPromptTemplate}\n\nBase Institucional:\n${compInfo}\n\n${hasNabIntelligence ? nabContext + '\n\n' : ''}Inventário:\n${formattedInventory}\n\n${constraintPrompt}`
    ...
    if (hasNabIntelligence && result.message) {
      ...
      if (msgL.includes('não aconteceu') || msgL.includes('futuro') || msgL.includes('ainda vai acontecer') || msgL.includes('não tenho informação')) {
        result.message = "Confirmamos diretamente da NAB 2026:\n\n" + intelligence.map((i: any) => i.ai_summary).join('\n\n');
