...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Build the productOrQuery using the top 8 most specific (longest) terms found in the query.
    let productOrQuery = '';
    if (relevantTerms.length > 0) {
      const topTerms = [...relevantTerms].sort((a: string, b: string) => b.length - a.length).slice(0, 8);
...
MANDATORY RULES:
- You are a Senior AV Solutions Expert. Your intelligence is governed by the 'intent_mapping' (Section H) and 'technical_bridge' (Section I) tables.
- Rule 1: Use Section H triggers to identify the user's core activity and prioritize those expanded keywords in your search.
- Rule 2: Use Section I bridges as your primary technical solution guide.
- Rule 3: You MUST use the full Inventory context to find the best matches. NEVER claim a product is missing if there are relevant items in the provided list.
- Rule 4: If multiple solutions exist for a technical requirement, you MUST present and explain all of them.
- Rule 5: For every product you name or recommend, you MUST include its exact ID in the 'referenced_internal_products' array.
