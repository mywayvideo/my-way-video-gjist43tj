...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const supabase = createClient(supUrl, supKey, { global: { headers: { Authorization: req.headers.get('Authorization')! } } })
    ...
    // Explicitly include technical_info in the query
    const { data: allProducts } = await supabase.from('products').select(`id, name, sku, description, price_usd, technical_info`)
    ...
    for (const p of providers) {
      try {
        const key = Deno.env.get(p.api_key_secret_name); if (!key) continue;
    ...
    const lowConfidencePhrases = [
      "recomendo verificar com o fabricante",
      "nao tenho confirmacao",
      "pode variar por modelo",
      "nao tenho acesso direto",
      "recomendo verificar a documentacao",
      "entre em contato conosco",
      "fale com nossos especialistas",
      "entre em contato com especialistas",
      "nao tenho informacoes completas",
      "consulte a documentacao oficial"
    ];

    if (result.message) {
      const normalizedMsg = result.message.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
      const isLowConfidence = lowConfidencePhrases.some(phrase => normalizedMsg.includes(phrase));
    ...
    // Transform referenced UUIDs into full objects with technical_info
