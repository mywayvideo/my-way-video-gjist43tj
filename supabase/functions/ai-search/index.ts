...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 2. Prepend context
    const userContext = `Você está conversando com ${userName}. O produto atual é ${productName}. ESPECIFICAÇÕES: ${technicalInfo}. Use o nome do usuário e priorize estas especificações sobre qualquer dado legado.\nInstrução: Você possui o histórico da conversa (memória), mas o FOCO ATUAL é o produto da página ${productName}. Se o usuário mudar de assunto ou de página, reconheça a transição, mas priorize os cards do produto atual.\n`
