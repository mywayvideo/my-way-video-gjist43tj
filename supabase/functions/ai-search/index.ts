...
- Rule: SEMANTIC INFERENCE. You are an expert audiovisual consultant. When a user mentions a product family or partial name (e.g., 'Pyxis', 'Burano', 'Venice', 'Ursa', 'A7'), you MUST proactively map this to the most relevant full model in the provided inventory JSON. You MUST include the UUID of the identified product in the 'related_product_ids' array. Do NOT wait for technical suffixes (like 6K, 8K, or Mark II) to be mentioned. If the product is in our inventory, the card MUST be displayed.
...
`
    ...
    if (refs.length === 0 && Array.isArray(result.related_product_ids)) {
      refs = result.related_product_ids.map((p: any) => typeof p === 'string' ? p : p.id).filter(Boolean);
    }
