            ...
            if (calls === 0) {
              ...
              if (providedIds.length > 0 || hasInvalidIds || hasSpecsOrOut) {
                ...
                msgs.push({
                  role: 'system',
                  content:
                    "SECURITY VIOLATION: You are FORBIDDEN from providing specs or claiming 'out of stock' without calling 'search_products' first. Call the tool now to verify the REAL inventory.",
                })
                calls++
                continue
              }
