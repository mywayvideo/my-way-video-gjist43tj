...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Explicitly remove terms that should never be ignored
    const wordsToRemove = ['camera', 'cameras', 'lente', 'lentes', 'modelo', 'marca', 'câmera'];
    wordsToRemove.forEach(w => stopWords.delete(w));
...
MANDATORY RULES:
- Rule 1 (Inventory Check): Before claiming a product is missing, you MUST perform a broad search. Never say 'I don't have cameras' if the stock list contains any camera model.
- Rule 2 (Workflow Logic): If a user asks for a solution (e.g., HDMI to Fiber) and the direct product only accepts SDI, you MUST search the catalog for an intermediate converter (HDMI to SDI) and recommend the complete chain. Explain the connection chain.
- Rule 3 (Card Trigger): You MUST use the EXACT product names from the database in your response. This is mandatory to trigger the UI cards.
...
- É OBRIGATÓRIO retornar TODOS os IDs (ou SKUs) exatos dos produtos do Inventário que você mencionou na resposta no array "referenced_internal_products". Isso é crítico para exibir as imagens na tela.${technicalBridgeRules}${tonePrompt}
...
`
    ...
    const resolvedRefs = refs.map((refId: string) => {
      const p = productsCtx.find((prod: any) => 
        prod.id === refId || 
        prod.sku === refId || 
        (prod.sku && prod.sku.toLowerCase().includes(refId.toLowerCase())) ||
        (prod.name && prod.name.toLowerCase().includes(refId.toLowerCase()))
      );
      if (p) {
        return p;
      }
      return refId; // Permite que o frontend busque esse ID/SKU de última hora
    }).filter(Boolean);
