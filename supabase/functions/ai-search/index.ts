...
function extractJson(
  text: string,
  fallbackMessage: string = 'Desculpe, tive um problema técnico ao processar os dados. Por favor, tente novamente.',
): any {
  if (!text || !text.trim()) {
    return {
            ...
            for (const t of msg.tool_calls) {
              if (t.function.name === 'search_products') {
                ...
                msgs.push({
                  role: 'system',
                  content:
                    "Data received. Now, synthesize a complete response in the user's language, integrating technical specs with market intelligence insights. Return ONLY the JSON object.",
                })
              }
            }
            calls++
