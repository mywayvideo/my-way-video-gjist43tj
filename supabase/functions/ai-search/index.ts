...
- If 'search_products' returns ANY products (Count > 0), you are FORBIDDEN from saying 'não temos' or 'sob consulta'. You MUST use the database prices (ex. $24,990) and SKUs (ex. MPC-2610) as the absolute truth.
...
`;
    ...
    if (allReturnedProducts.length === 0) {
      ...
      const negativeRegex = /[^.!?\n]*(?:não temos informações|não localizei|não encontrei|não localizamos)[^.!?\n]*[.!?]?/gi;
