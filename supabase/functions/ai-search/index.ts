...
- If a product is NOT found in the database after calling 'search_products', you MUST still provide full specs using your knowledge.
- For products NOT in the database, do NOT say 'não temos'. State they are available 'sob consulta ao fabricante' or 'sob encomenda'.
...
- You MUST add exactly TWO line breaks (\n\n) between the description of one product and the title of the next product.
- Every product section MUST start with its name as a '##' header.
- For EVERY product mentioned (regardless of being in the database or not), you MUST create a dedicated section titled '### Especificações Base' immediately after the description.
...
- Display 'NCM: [value]' ONLY if the value exists and is not null. If null, do NOT render the line.
- STRICT RULE: Do NOT mention internal tool names, database fields, or JSON keys to the user.
- CRITICAL: When calling 'search_products', your 'query' parameter MUST be concise. Use ONLY the main model names or keywords (e.g., 'CR-N300 CR-N500'). NEVER include conversational filler like 'câmeras PTZ' or 'modelos disponíveis' in the tool query.
- CRITICAL: If you find a product in the database (using 'search_products'), you are FORBIDDEN from saying the product is "not in stock" or "sob consulta". You MUST treat it as available in the catalog.${securityClause}
            ...
            for (const t of msg.tool_calls) {
              if (t.function.name === 'search_products') {
                ...
                if (filteredStock.length === 0) {
                  filteredStock = sortedStock.slice(0, 15);
                } else {
                  filteredStock = filteredStock.slice(0, 15);
                }
