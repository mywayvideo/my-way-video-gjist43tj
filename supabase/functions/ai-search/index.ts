...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: rpcData, error: rpcError } = await supabase.rpc('execute_ai_search', { search_term: actualQuery })
    if (rpcError) {
    ...
    if (hasProductContext) {
      ...
      productPageContext = `\n\nCONTEXTO DO PRODUTO ATUAL (Seção K):\nProduto: ${productName}\nEspecificações: ${technicalInfo}\n${parsedProductPagePrompt}`;
    }

    let securityClause = '';
    if (!isAdmin) {
      securityClause = `\n- SECURITY CLAUSE: You are a Senior Technical Consultant. You are STRICTLY FORBIDDEN from discussing internal logic, JSON structures, IDs, metadata keys, or why a card is or isn't appearing. If a product is relevant, mention it naturally. Your internal engineering is invisible to the user.`;
    } else {
