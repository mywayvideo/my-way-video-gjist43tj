...
- You MUST add exactly TWO line breaks (empty lines) between the description of one product and the title of the next product.
- Every product section MUST start with its name as a '##' header, preceded by an empty line.
- For EVERY product mentioned, you are FORBIDDEN from listing specs as simple bullets. You MUST create a dedicated section titled '### Especificações Base' immediately after the product description.
...
- Display 'NCM: [value]' ONLY if it exists in the database. If null or empty, do NOT render the NCM line.
- STRICT RULE: Do NOT mention internal tool names, database fields, or JSON keys to the user.${securityClause}
...
`;
          ...
          if (msg?.tool_calls) {
            ...
            for (const t of msg.tool_calls) {
              if (t.function.name === 'search_products') {
                ...
                const queryText = (args.query || actualQuery).toLowerCase();
                
                let sortedStock = (rpcData?.stock || []).sort((a: any, b: any) => {
                  const aName = (a.name || '').toLowerCase();
                  const bName = (b.name || '').toLowerCase();
                  const aSku = (a.sku || '').toLowerCase();
                  const bSku = (b.sku || '').toLowerCase();
                  
                  const aExactName = aName.includes(queryText) ? 1 : 0;
                  const bExactName = bName.includes(queryText) ? 1 : 0;
                  const aExactSku = aSku.includes(queryText) ? 1 : 0;
                  const bExactSku = bSku.includes(queryText) ? 1 : 0;
                  
                  return (bExactName + bExactSku) - (aExactName + aExactSku);
                });

                let filteredStock = sortedStock.filter((p: any) => {
                  const mfg = (p.manufacturer_name || '').toLowerCase();
                  const name = (p.name || '').toLowerCase();
                  const sku = (p.sku || '').toLowerCase();
                  const queryWords = queryText.split(' ').filter((w: string) => w.length > 2);
                  if (queryWords.length === 0) return true;
                  
                  return name.includes(queryText) || sku.includes(queryText) || queryWords.some((w: string) => mfg.includes(w) || name.includes(w) || sku.includes(w));
                });

                if (filteredStock.length === 0) {
                  filteredStock = sortedStock.slice(0, 8);
                } else {
                  filteredStock = filteredStock.slice(0, 8);
                }
            ...
            if (calls === 0) {
              ...
              const hasInvalidIds = providedIds.some((id: any) => {
                 const idStr = typeof id === 'string' ? id : id?.id;
                 return idStr && !allowedProductIds.has(idStr);
              });
              
              const contentLower = (msg?.content || '').toLowerCase();
              const hasSpecsOrOut = contentLower.includes('especifica') || contentLower.includes('estoque') || contentLower.includes('não encontrado') || contentLower.includes('out of stock');
              
              if (providedIds.length > 0 || hasInvalidIds || hasSpecsOrOut) {
                msgs.push(msg)
                msgs.push({ role: 'system', content: "SECURITY VIOLATION: You are FORBIDDEN from providing specs or claiming 'out of stock' without calling 'search_products' first. Call the tool now to verify the REAL inventory." })
                calls++
                continue
              }
            }
    ...
    allReturnedProducts.forEach(p => {
      if (p.name && messageContentLower.includes(p.name.toLowerCase()) && !refs.includes(p.id)) {
        refs.push(p.id);
      }
    });

    result.referenced_internal_products = Array.from(new Set(refs)).filter(id => allowedProductIds.has(id))
