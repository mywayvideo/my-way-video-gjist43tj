...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: allProducts } = await supabase.from('products').select(`id, name, sku, description, price_usd`)
    ...
    supabase.from('product_search_cache').insert({ search_query: query, product_name: 'AI Match', source: 'ai_generated', product_description: result.message }).then()
