...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let sysPromptTemplate = aiSettings?.system_prompt_template || '';
    let finalBasePrompt = sysPromptTemplate.trim() 
      ? sysPromptTemplate.replace('{{system_prompt}}', settings.system_prompt || '') 
      : (settings.system_prompt || 'VOCÊ É UM CONSULTOR ESPECIALISTA MY WAY BUSINESS.');
    ...
    const sysPrompt = `### MANDATORY RULES: HIGHEST AUTHORITY ###
