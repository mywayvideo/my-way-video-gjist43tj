...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (discount_amount > total_profit_margin) {
      return new Response(JSON.stringify({ error: `O valor do desconto ultrapassa a margem de lucro da empresa. O lucro total disponível para este pedido é de USD ${total_profit_margin.toFixed(2)}.` }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
