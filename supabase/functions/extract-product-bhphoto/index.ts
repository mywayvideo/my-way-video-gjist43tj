...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    while (attempt < maxAttempts && !fetchSuccess) {
      attempt++;
      try {
        console.log(`[extract-product-bhphoto] Calling ScrapingBee API (Attempt ${attempt}/${maxAttempts})...`);
        ...
        if (response.status === 429) {
          console.warn(`[extract-product-bhphoto] ScrapingBee Rate limit exceeded (429) on attempt ${attempt}`);
        ...
        if (!response.ok) {
          ...
          console.error(`[extract-product-bhphoto] ScrapingBee HTTP error! Status: ${response.status}, Response: ${errorText}`);
    ...
    if (!fetchSuccess) {
      console.error(`[extract-product-bhphoto] Failed to fetch from ScrapingBee after ${maxAttempts} attempts. Last error:`, lastError?.message);
      return new Response(JSON.stringify({ error: 'Nao foi possivel acessar a pagina da B&H apos multiplas tentativas de rede. Tente novamente mais tarde.' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
    ...
    console.log(`[extract-product-bhphoto] Extracted raw data -> Name: ${name}, SKU: ${sku}, Price: ${price_usa}, Weight: ${weight}, Dim: ${dimensions}, Category: ${category}`);

    const apiKey = Deno.env.get('OPENAI_API_KEY');
    if (!apiKey) {
      console.warn(`[extract-product-bhphoto] OPENAI_API_KEY not found. LLM formatting will be skipped.`);
    }
...
Return ONLY a JSON object with keys: "description", "technical_info", "category".`
            ...
            if (llmRes.ok) {
                ...
                console.error(`[extract-product-bhphoto] OpenAI error status: ${llmRes.status}, body: ${llmErr}`);
            }
        } catch (e: any) {
      ...
      if (supabaseUrl && supabaseKey && category) {
        ...
        if (dbErr) {
          ...
          if (matchedCategory) {
            ...
            console.log(`[extract-product-bhphoto] Matched category ID: ${category_id} (${matchedCategory.name})`);
          } else {
            console.log(`[extract-product-bhphoto] No matching category found for: ${category}`);
          }
        }
      } else {
        if (!supabaseUrl || !supabaseKey) console.warn(`[extract-product-bhphoto] Missing Supabase credentials for category matching`);
      }
    } catch (e: any) {
      console.error(`[extract-product-bhphoto] Error fetching categories from DB:`, e.message, e.stack);
    }
    ...
    console.log(`[extract-product-bhphoto] Extraction complete. Returning result for ${sku || name}`);
    ...
    console.error(`[extract-product-bhphoto] Unhandled error for URL ${url}:`, error.message, error.stack);
