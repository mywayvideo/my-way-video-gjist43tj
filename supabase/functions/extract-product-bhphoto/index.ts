  ...
  try {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error(`[extract-product-bhphoto] Apify HTTP error! Status: ${res.status}, Response: ${errorText}`);
    ...
    if (item.dimensions) {
      ...
      const dimMatch = dStr.match(/([\d.]+)[\s"in]*(?:x|X)[\s]*([\d.]+)[\s"in]*(?:x|X)[\s]*([\d.]+)/i);
      if (dimMatch) {
    ...
    console.log(`[extract-product-bhphoto] Raw extracted: Name: ${name}, SKU: ${sku}, Price: ${price_usa}`);
