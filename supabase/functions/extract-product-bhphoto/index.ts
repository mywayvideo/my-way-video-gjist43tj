    ...
    try {
      ...
      if (supabaseUrl && supabaseKey && manufacturer) {
        ...
        if (dbErr) {
          ...
          const matchedMfg = dbMfgs.find(
            (m: any) =>
              mfgNameLower.includes(m.name.toLowerCase()) ||
              m.name.toLowerCase().includes(mfgNameLower),
          )
          if (matchedMfg) {
            manufacturer_id = matchedMfg.id
            console.log(
              `[extract-product] Matched manufacturer ID: ${manufacturer_id} (${matchedMfg.name})`,
            )
          } else {
