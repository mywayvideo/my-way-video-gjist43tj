      ...
      const res = await fetch(firecrawlUrl, {
        ...
        body: JSON.stringify({
          ...
          formats: [{
            ...
            prompt: "Extract the product name, sku (prefer the Manufacturer's code, e.g. MFR # over the store's internal SKU), manufacturer name, price in USD (only numbers), description (CRITICAL: extract ONLY from the 'Overview' section/tab), technical specifications (as markdown), weight (CRITICAL: extract ONLY 'Package Weight' from the 'Packaging Info' block within the 'Specs' tab. Ignore general physical specifications weight), dimensions (CRITICAL: extract ONLY 'Box Dimensions (LxWxH)' from the 'Packaging Info' block within the 'Specs' tab. Ignore general physical dimensions. Remove trailing quotes or inch symbols), main image URL, category, whether it is discontinued, and stock availability."
          }],
...
Return ONLY a JSON object with keys: "description", "technical_info", "category".`
    ...
    let manufacturer_id = "";
    try {
      console.log(`[extract-product] Matching manufacturer in DB: ${manufacturer}`);
      const supabaseUrl = Deno.env.get('SUPABASE_URL') || '';
      const supabaseKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') || '';
      if (supabaseUrl && supabaseKey && manufacturer) {
        const supabase = createClient(supabaseUrl, supabaseKey);
        
        const { data: dbMfgs, error: dbErr } = await supabase.from('manufacturers').select('id, name');
        if (dbErr) {
           console.error(`[extract-product] Error querying manufacturers: ${dbErr.message}`);
        } else if (dbMfgs && dbMfgs.length > 0) {
          const mfgNameLower = String(manufacturer).toLowerCase();
          const matchedMfg = dbMfgs.find((m: any) => 
            mfgNameLower.includes(m.name.toLowerCase()) || m.name.toLowerCase().includes(mfgNameLower)
          );
          if (matchedMfg) {
            manufacturer_id = matchedMfg.id;
            console.log(`[extract-product] Matched manufacturer ID: ${manufacturer_id} (${matchedMfg.name})`);
          } else {
            console.log(`[extract-product] No matching manufacturer found for: ${manufacturer}`);
          }
        }
      }
    } catch (e: any) {
