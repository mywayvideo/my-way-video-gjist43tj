...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let html = '';
    try {
      console.log(`[extract-product-bhphoto] Calling ScrapingBee API...`);
      ...
      if (response.status === 429) {
        console.warn(`[extract-product-bhphoto] ScrapingBee Rate limit exceeded (429)`);
      ...
      console.error(`[extract-product-bhphoto] Fetch error for URL ${url}:`, error.message, error.stack);
