  ...
  try {
    ...
    const evaluateWhatsAppTriggers = async (
      ...
      if (triggerPurchase && !showBtn) {
        ...
        if (
          purchaseKeywords.some((kw: string) =>
      ...
      if (triggerProject && !showBtn) {
        ...
        if (
          projectKeywords.some((kw: string) =>
            normalizedResponse.includes(
              kw
                .toLowerCase()
                .normalize('NFD')
...
5. Paragraphs: Maximum 2 sentences.`
    ...
    if (success) {
      ...
      const normalizedForTrigger = responseText
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
      if (
      ...
      let should_show_whatsapp_button = await evaluateWhatsAppTriggers(
        responseText,
        refIds,
        finalConfidence,
      )
