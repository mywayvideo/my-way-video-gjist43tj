...
FORMATTING RULES: NEVER write paragraphs longer than 2 lines. ALWAYS break long text into bullets. ALWAYS separate sections with blank lines.`
    ...
    if (success) {
        ...
        try {
            ...
            if (queryLower.includes('camera') || queryLower.includes('câmera') || queryLower.includes('video')) activeContexts.push(['camera', 'video', 'cinema'])
            if (queryLower.includes('lens') || queryLower.includes('lente')) activeContexts.push(['lens', 'accessory', 'lentes'])
            if (queryLower.includes('tripod') || queryLower.includes('tripé')) activeContexts.push(['tripod', 'accessory', 'suporte'])
            if (queryLower.includes('audio') || queryLower.includes('áudio') || queryLower.includes('mic')) activeContexts.push(['audio', 'accessory'])
            if (queryLower.includes('monitor') || queryLower.includes('tela')) activeContexts.push(['monitor', 'video'])
            if (queryLower.includes('light') || queryLower.includes('luz') || queryLower.includes('iluminação')) activeContexts.push(['lighting', 'iluminação', 'accessory'])
            ...
            for (const product of allProducts) {
                ...
                if (product.sku) {
                    variations.add(product.sku.toLowerCase().trim())
                    if (product.sku.includes('-')) variations.add(product.sku.replace(/-/g, '').toLowerCase().trim())
                }

                if (words.length > 1) {
                    const brand = words[0]
                    const model = words.slice(1).join(' ')
                    if (model.length >= 3) variations.add(model)
                    
                    const lastWord = words[words.length - 1]
                    if (lastWord.length >= 2) variations.add(`${brand} ${lastWord}`)

                    words.forEach((w: string) => {
                        if (w.length >= 3 && /\d/.test(w)) {
                            variations.add(w)
                            if (brand !== w) variations.add(`${brand} ${w}`)
                        }
                    })
                }
                ...
                for (const variation of variations) {
                    ...
                    const boundary = `(^|[\\s,.;:!?'"()\\[\\]{}<>-])`
                    const regex = new RegExp(`${boundary}${escapeRegExp(variation)}${boundary}`, 'i')
                ...
                if (isMatch) {
                    ...
                    if (activeContexts.length > 0 && !isExactMatch) {
                        const matchesAnyActiveContext = activeContexts.some(allowedCats => 
                            allowedCats.some(c => pCatLower.includes(c))
                        )
                        if (!matchesAnyActiveContext) {
