        ...
        try {
            ...
            for (const product of allProducts) {
                    ...
                    if (words.length > 1) {
                        ...
                        for (const token of modelTokens) {
                            if (token.length >= 2) {
                                ...
                                const brandModelRegex = new RegExp(`\\b${escapeRegExp(brand)}\\s+${escapeRegExp(token)}\\b`, 'i')
                ...
                if (isMatch) {
                    ...
                    const askedAboutCameraOrVideo = queryLower.includes('camera') || queryLower.includes('câmera') || queryLower.includes('video') || queryLower.includes('vídeo')
                    
                    if (askedAboutCameraOrVideo) {
                        const hasCameraOrVideoCategory = pCatLower.includes('camera') || pCatLower.includes('câmera') || pCatLower.includes('video') || pCatLower.includes('vídeo') || pCatLower.includes('cinema')
