    ...
    if (success) {
      ...
      try {
        ...
        if (
          ...
          activeContexts.push(['camera', 'video', 'cinema'])
        if (queryLower.includes('lens') || queryLower.includes('lente'))
          activeContexts.push(['lens', 'accessory', 'lentes'])
        if (queryLower.includes('tripod') || queryLower.includes('tripé'))
          activeContexts.push(['tripod', 'accessory', 'suporte'])
        if (
          queryLower.includes('audio') ||
          queryLower.includes('áudio') ||
          queryLower.includes('mic')
        )
          activeContexts.push(['audio', 'accessory'])
        if (queryLower.includes('monitor') || queryLower.includes('tela'))
          activeContexts.push(['monitor', 'video'])
        if (
          queryLower.includes('light') ||
          queryLower.includes('luz') ||
          queryLower.includes('iluminação')
        )
        ...
        for (const product of allProducts) {
          ...
          if (product.sku) {
            ...
            if (product.sku.includes('-'))
              variations.add(product.sku.replace(/-/g, '').toLowerCase().trim())
          }
