...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
        const { data: setts } = await supabaseAdmin.from('ai_settings').select('*').limit(1).maybeSingle();
    ...
    if (aiSettingsData?.technical_bridge) {
      try {
        const bridges = typeof aiSettingsData.technical_bridge === 'string' ? JSON.parse(aiSettingsData.technical_bridge) : aiSettingsData.technical_bridge;
  ...
  "referenced_internal_products": ["id_1", "id_2"]
    ...
    if (success) {
        ...
        if (refs.length === 0 && Array.isArray(parsedResponse.products)) {
          refs = parsedResponse.products.map((p: any) => typeof p === 'string' ? p : p.id).filter(Boolean);
        }
