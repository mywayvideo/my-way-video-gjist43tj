...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // CACHE LOGIC TEMPORARILY DISABLED FOR DEBUGGING
    /*
    ...
    */
    ...
    let allProducts: any[] = []
    try {
      const { data: prodData } = await supabaseAdmin.from('products').select('id, name, sku, category, price_usd, image_url')
    ...
    if (success) {
        ...
        try {
            ...
            for (const product of allProducts) {
                ...
                console.log(`[${tsLog}] Checking product: ${product.name} | Category: ${product.category || 'none'} | Aliases: [${aliasArr.join(', ')}]`);
                ...
                if (aliasMatched && contextMatched) {
                    console.log(`[${tsLog}]   - Result: INCLUDE (Alias '${matchedAliasStr}' matched and category valid)`);
            ...
            console.log(`[${tsLog}] Final matched products: ${matchedProducts.length} | IDs: [${matchedProducts.map(p => p.id).join(', ')}]`);

            if (matchedProducts.length === 0) {
                console.log(`[${tsLog}] WARNING: No products matched despite ${allProducts.length} total products in database`);
            }
        ...
        // CACHE INSERT LOGIC TEMPORARILY DISABLED FOR DEBUGGING
        /*
        try {
