        ...
        try {
            ...
            for (const product of allProducts) {
                    ...
                    if (words.length > 1) {
                        ...
                        for (const token of modelTokens) {
                            if (token.length >= 2) {
                                ...
                                const tokensToTest = [token]
                                
                                // 4. Suffix removal: if product is FX3A, also match FX3
                                if (/\d/.test(token) && /[a-z]+$/i.test(token)) {
                                    const baseToken = token.replace(/[a-z]+$/i, '')
                                    if (baseToken.length >= 2) {
                                        tokensToTest.push(baseToken)
                                    }
                                }

                                for (const t of tokensToTest) {
                                    const tokenRegex = new RegExp(`\\b${escapeRegExp(t)}\\b`, 'i')
                                    
                                    // 2. Product model number (e.g., R5 matches Canon EOS R5)
                                    if (/\d/.test(t) && tokenRegex.test(responseTextLower)) {
                                        isMatch = true
                                        break
                                    }

                                    // 3. Brand plus model (e.g., Sony plus FX3)
                                    const brandModelRegex = new RegExp(`\\b${escapeRegExp(brand)}\\s+${escapeRegExp(t)}\\b`, 'i')
                ...
                if (isMatch) {
                    ...
                    const askedAboutCameraOrVideo = queryLower.includes('camera') || 
                                                    queryLower.includes('câmera') || 
                                                    queryLower.includes('video') || 
                                                    queryLower.includes('vídeo') ||
                                                    queryLower.includes('codec') ||
                                                    queryLower.includes('frame rate') ||
                                                    queryLower.includes('gravacao') ||
                                                    queryLower.includes('gravação')
                    
                    if (askedAboutCameraOrVideo) {
                        const hasCameraOrVideoCategory = pCatLower.includes('camera') || 
