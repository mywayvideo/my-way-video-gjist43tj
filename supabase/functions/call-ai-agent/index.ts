...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const productMap = allProducts.map(p => {
      ...
      parts.forEach(part => {
        if (part.length >= 3 && !['com', 'para', 'dos', 'das', 'kit', 'pro', 'the', 'and', 'for', 'uma', 'um', 'camera', 'câmera', 'lente', 'lenses'].includes(part)) {
          aliases.add(part)
        }
      })
...
**Recomendação Final:** The Sachtler 1018AM is an excellent choice for Burano. Ensure drag and counterbalance are properly calibrated.`
    ...
    for (const provider of providers) {
      ...
      if (success) {
        ...
        try {
          const stopWords = new Set(['com', 'para', 'dos', 'das', 'kit', 'pro', 'the', 'and', 'for', 'uma', 'um', 'que', 'qual', 'quais', 'sobre', 'este', 'esta', 'esse', 'essa', 'como', 'quando', 'onde', 'porque', 'por que', 'tem'])
          ...
          const extractPhrases = (text: string) => {
              ...
              return Array.from(new Set([...words, ...createNgrams(words, 2), ...createNgrams(words, 3)]))
          }
          ...
          const matchPhrases = (phrases: string[], matchedNames: Set<string>) => {
              for (const phrase of phrases) {
                  ...
                  for (const prod of productMap) {
                      ...
                      if (prod.aliases.includes(phrase) || prod.nameLower === phrase || prod.skuLower === phrase) {
