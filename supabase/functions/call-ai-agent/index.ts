      ...
      try {
        ...
        for (const product of allProducts) {
          ...
          if (contextMatched) {
            ...
            logsChecked.push(
              `[${product.name}]: Exclude (Alias matched '${matchedAliasStr}', but category '${pCat}' mismatched context)`,
            )
          }
        }
        ...
        console.log(
          `[Semantic Context] Final matched products:`,
          matchedProducts.map((p) => ({ id: p.id, name: p.name })),
        )
      } catch (err) {
