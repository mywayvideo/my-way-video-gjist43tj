            ...
            productMap.forEach(prod => {
                ...
                if (isMentioned) {
                    ...
                    if (contextMatch || combinedContexts.length === 0) {
                        ...
                        productsChecked.push(`${prod.name}: INCLUDE (Alias matched, Context matched: ${prod.category})`)
                    } else {
                        productsChecked.push(`${prod.name}: EXCLUDE (Alias matched, but Context DID NOT match: ${prod.category})`)
                    }
                }
            })
