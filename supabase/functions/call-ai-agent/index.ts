...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let allProducts: any[] = []
    try {
      const { data: prodData } = await supabaseAdmin.from('products').select('id, name, sku, category, price_usd, price_cost, price_nationalized_sales, price_nationalized_cost, price_nationalized_currency, weight, category_id, manufacturer_id, image_url, is_discontinued')
...
FORMATTING RULES: NEVER write paragraphs longer than 2 sentences. ALWAYS format technical specs in markdown code blocks (\`\`\`). ALWAYS include the official Brazil/LATAM warranty disclaimer at the end. ALWAYS separate sections with blank lines.`
