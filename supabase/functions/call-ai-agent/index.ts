...
FORMATTING RULES: NEVER write paragraphs longer than 2 lines. ALWAYS break long text into bullets. ALWAYS separate sections with blank lines.`
    ...
    if (success) {
        ...
        try {
            ...
            console.log(`[Semantic Context] Semantic contexts found:`, foundContexts);

            const logsChecked: string[] = [];
            ...
            for (const product of allProducts) {
                ...
                if (!aliasMatched) {
                    logsChecked.push(`[${product.name}]: No alias match`);
                    continue;
                }
                ...
                if (contextMatched) {
                    logsChecked.push(`[${product.name}]: Match (Alias: ${matchedAliasStr}, Context: valid)`);
                    matchedProductsRaw.push(product);
                } else {
                    logsChecked.push(`[${product.name}]: Exclude (Alias matched '${matchedAliasStr}', but category '${pCat}' mismatched context)`);
                }
            }

            console.log(`[Semantic Context] Products checked:\n` + logsChecked.join('\n'));
            ...
            console.log(`[Semantic Context] Final matched products:`, matchedProducts.map(p => ({ id: p.id, name: p.name })));
