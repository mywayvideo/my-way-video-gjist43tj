...
function getHeuristicIntent(query: string, currentProductId: string | null): string {
  ...
  const institutionalPattern =
    /\b(horari\w*|localiz\w*|enderez\w*|enderec\w*|contat\w*|garanti\w*|logist\w*|funcionament\w*|quem somos|onde fic\w*|address\w*|hours|location\w*|sobre|about us|quienes somos|ubicacion|atendimento|aberto|fecha\w*|opening hours|warranty|envio\w*|frete\w*|shipping|whatsapp|telefone\w*|email|contact\w*)\b/i
...
function buildDynamicSystemPrompt(
  ...
  // 4. Logística + Contexto da Empresa (agora sempre incluso)
  ...
  const companyContext = Array.isArray(companyInfo) 
    ? companyInfo.map((c: any) => c.content || c.transparency_note || '').filter(Boolean).join('\n\n')
  ...
  try {
    ...
    if (intent === 'INSTITUTIONAL') {
      const allInfo = Array.isArray(companyInfo) 
        ? companyInfo.map((c: any) => c.content).filter(Boolean).join('\n\n') 
        : (companyInfo?.content || 'Somos a My Way Video.');

      const aiProviders = await getActiveAIProviders(supabase);
      const systemPrompt = `Você é o assistente virtual da My Way Video.
Sua tarefa é responder à pergunta do usuário de forma amigável usando EXCLUSIVAMENTE as seguintes informações institucionais:

${allInfo}

Regras:
1. Responda de forma clara, amigável e direta.
2. Se a informação não estiver no texto acima, diga "Desculpe, não tenho essa informação no momento. Por favor, entre em contato via WhatsApp."
3. Não invente dados.`;

      let aiMessage = allInfo;
      try {
        ...
        if (normalizedData?.choices?.[0]?.message?.content) {
          aiMessage = normalizedData.choices[0].message.content;
        }
      } catch (err) {
        console.error('[INSTITUTIONAL_AI_ERROR]', err);
      }
