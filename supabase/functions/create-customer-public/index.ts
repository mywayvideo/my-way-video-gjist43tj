...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!hcaptchaSecret) {
      return new Response(JSON.stringify({ error: 'Erro na configuracao do servidor.' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (!hcaptchaData.success) {
      return new Response(JSON.stringify({ error: 'Verificacao de humano falhou. Tente novamente.' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }

    const { data: existingUser } = await supabaseAdmin.from('customers').select('id').eq('email', email).maybeSingle()
    if (existingUser) {
    ...
    return new Response(JSON.stringify({ error: 'Não foi possível criar conta.' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  }
})
