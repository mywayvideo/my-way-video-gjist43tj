...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const hcaptchaSecret = Deno.env.get('HCAPTCHA_SECRET_KEY') || '0x0000000000000000000000000000000000000000'
    ...
    if (!hcaptchaData.success) {
      return new Response(JSON.stringify({ error: 'Verificação de humano falhou.' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
