...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!model_id || !api_key_secret_name || !endpoint || !provider_type) {
      console.log(`[${timestamp}] provider_type: ${provider_type || 'unknown'}, model_id: ${model_id || 'unknown'}, endpoint: ${endpoint || 'unknown'}, request status: failed_validation, response status: 400`)
    ...
    if (apiKey === undefined || apiKey === null) {
      console.log(`[${timestamp}] provider_type: ${provider_type}, model_id: ${model_id}, endpoint: ${endpoint}, request status: missing_secret, response status: 400`)
      return new Response(JSON.stringify({
        status: 'error',
    ...
    if (apiKey.trim() === '') {
      console.log(`[${timestamp}] provider_type: ${provider_type}, model_id: ${model_id}, endpoint: ${endpoint}, request status: empty_secret, response status: 400`)
      return new Response(JSON.stringify({
        status: 'error',
    ...
    if (fetchError && fetchError.name === 'AbortError') {
      console.log(`[${logTimestamp}] provider_type: ${provider_type}, model_id: ${model_id}, endpoint: ${endpoint}, request status: timeout, response status: 504`)
      return new Response(JSON.stringify({
        status: 'error',
    ...
    if (!finalResponse) {
      console.log(`[${logTimestamp}] provider_type: ${provider_type}, model_id: ${model_id}, endpoint: ${endpoint}, request status: fetch_error, response status: 500`)
      return new Response(JSON.stringify({
        status: 'error',
    ...
    console.log(`[${logTimestamp}] provider_type: ${provider_type}, model_id: ${model_id}, endpoint: ${endpoint}, request status: completed, response status: ${statusCode}`)
    ...
    }), { status: statusCode >= 500 ? 502 : 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })

  } catch (err: any) {
    console.log(`[${new Date().toISOString()}] provider_type: unknown, model_id: unknown, endpoint: unknown, request status: fatal_error, response status: 500`)
    return new Response(JSON.stringify({
      status: 'error',
