...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!email || !password || password.length < 8 || !name) {
      return new Response(JSON.stringify({ error: 'Preencha todos os campos obrigatórios' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (createError) {
      if (createError.message.toLowerCase().includes('already registered') || createError.message.toLowerCase().includes('already exists')) {
        return new Response(JSON.stringify({ error: 'Este e-mail já está cadastrado' }), { status: 409, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
      }
      throw createError
    }
    ...
    const { error: updateError } = await supabaseAdmin.from('customers').update(customerObj).eq('user_id', userId)
    ...
    return new Response(JSON.stringify({ success: true, customer_id: userId }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    ...
    return new Response(JSON.stringify({ error: 'Erro ao salvar cliente. Tente novamente.', details: err.message }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  }
})
