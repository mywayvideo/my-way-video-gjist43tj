...
//     search_words text[];
//   BEGIN
//     SELECT array_agg(DISTINCT trim(w)) INTO search_words
//     FROM regexp_split_to_table(
//       regexp_replace(
//         lower(search_term),
//         '\m(?:camera|sony|canon|nikon|8k|4k)\M',
//         '',
//         'gi'
//       ),
//       '\s+'
//     ) w
//     WHERE length(trim(w)) > 1;
//   
//     RETURN COALESCE(
//       to_jsonb(array_agg(
//         jsonb_build_object(
//           'stock', p.stock,
//           'intel', COALESCE(p.intel, '{}'::jsonb),
//           'nab_data', COALESCE(p.nab_data, '{}'::jsonb)
//         )
//         ORDER BY 
//           CASE 
//             WHEN EXISTS(SELECT 1 FROM unnest(search_words) AS w WHERE p.sku ILIKE '%' || w || '%') 
//             THEN 1 ELSE 0 
//           END DESC,
