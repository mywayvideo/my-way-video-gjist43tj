...
//             WHEN (SELECT count(*) FROM parsed) > 0 AND (SELECT count(*) FROM parsed WHERE p.name ILIKE '%' || word || '%' OR p.sku ILIKE '%' || word || '%') = (SELECT count(*) FROM parsed) THEN 60
...
//             ELSE COALESCE((SELECT count(*) FROM parsed WHERE p.name ILIKE '%' || word || '%' OR p.sku ILIKE '%' || word || '%'), 0) * 5
//           END DESC,
//           p.price_usd DESC
//         LIMIT 30
//       ) sub
...
//           OR event_name ILIKE '%NAB 2026%'
//           OR event_name ILIKE '%' || (SELECT raw_term FROM search) || '%'
//         )
//         LIMIT 3
//       ) c
//     ),
...
//         WHERE (
//           title ILIKE '%' || (SELECT raw_term FROM search) || '%' 
//           OR content ILIKE '%' || (SELECT raw_term FROM search) || '%'
//           OR title ILIKE '%NAB%' 
//           OR content ILIKE '%NAB%'
//         ) 
//         LIMIT 5
//       ) n
//     )
