...
//   proactivity_level: integer (nullable, default: 5)
// Table: ai_providers
//   id: uuid (not null, default: gen_random_uuid())
...
//   id: bigint (not null, default: nextval('ai_rate_limits_id_seq'::regclass))
...
//   search_algorithm_sql: text (nullable)
//   system_prompt_template: text (nullable)
//   result_component_config: jsonb (nullable, default: '{}'::jsonb)
//   cache_expiration_days: integer (nullable, default: 30)
//   price_threshold_usd: numeric (nullable, default: 5000.00)
//   ignore_stock_count: boolean (nullable, default: true)
//   logistics_rules_prompt: text (nullable, default: 'Se price_usd > 0: Miami e Brasil. Se apenas price_nationalized_sales > 0: Somente Brasil.'::text)
//   intent_mapping: jsonb (nullable, default: '{}'::jsonb)
//   technical_bridge: jsonb (nullable, default: '{}'::jsonb)
//   custom_stop_words: text (nullable)
//   product_page_prompt: text (nullable)
// Table: app_settings
//   id: uuid (not null, default: gen_random_uuid())
...
// Table: avpro_keywords
//   keyword: text (not null)
//   category: text (nullable)
//   weight: numeric (nullable, default: 1.0)
//   is_blocking: boolean (nullable, default: false)
//   added_by: text (nullable, default: 'system'::text)
//   updated_at: timestamp with time zone (nullable, default: now())
// Table: cache_settings
//   id: uuid (not null, default: gen_random_uuid())
//   mi_expiration_days: integer (not null, default: 365)
//   product_search_cache_expiration_days: integer (not null, default: 30)
//   product_cache_expiration_days: integer (not null, default: 90)
//   updated_at: timestamp with time zone (nullable, default: now())
// Table: cart_items
//   id: uuid (not null, default: gen_random_uuid())
...
//   has_migrated: boolean (nullable, default: false)
...
//   target_type: text (nullable, default: 'specific'::text)
//   excluded_products: _uuid (nullable, default: '{}'::uuid[])
//   customers: _uuid (nullable, default: '{}'::uuid[])
//   manufacturer_ids: _uuid (nullable, default: '{}'::uuid[])
//   category_ids: _uuid (nullable, default: '{}'::uuid[])
// Table: exchange_rate
//   id: uuid (not null, default: gen_random_uuid())
...
//   event_name: text (nullable, default: 'NAB 2026'::text)
//   status: text (nullable, default: 'draft'::text)
//   created_at: timestamp with time zone (not null, default: timezone('utc'::text, now()))
//   metadata: jsonb (nullable, default: '{}'::jsonb)
//   expires_at: timestamp with time zone (nullable, default: (now() + '1 year'::interval))
// Table: nab_market
//   id: uuid (not null, default: gen_random_uuid())
//   title: text (not null)
...
//   category_id: uuid (nullable)
//   manual_related_ids: _uuid (nullable, default: '{}'::uuid[])
//   ai_related_ids: _uuid (nullable, default: '{}'::uuid[])
//   price_nationalized_sales: numeric (nullable)
//   price_nationalized_cost: numeric (nullable)
//   price_nationalized_currency: text (not null, default: 'BRL'::text)
//   rejected_related_ids: _uuid (nullable, default: '{}'::uuid[])
//   price_usa_rebate: numeric (nullable)
//   price_cost_rebate: numeric (nullable)
...
// Table: ai_rate_limits
//   PRIMARY KEY ai_rate_limits_pkey: PRIMARY KEY (id)
// Table: ai_settings
//   PRIMARY KEY ai_settings_pkey: PRIMARY KEY (id)
// Table: app_settings
...
// Table: avpro_keywords
//   PRIMARY KEY avpro_keywords_pkey: PRIMARY KEY (keyword)
// Table: cache_settings
//   PRIMARY KEY cache_settings_pkey: PRIMARY KEY (id)
// Table: cart_items
...
//   CHECK chat_messages_role_check: CHECK ((role = ANY (ARRAY['user'::text, 'assistant'::text])))
//   FOREIGN KEY chat_messages_user_id_fkey: FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE
// Table: company_info
...
// Table: customers
//   UNIQUE customers_email_key: UNIQUE (email)
...
// Table: market_intelligence
//   FOREIGN KEY market_intelligence_manufacturer_id_fkey: FOREIGN KEY (manufacturer_id) REFERENCES manufacturers(id) ON DELETE SET NULL
//   PRIMARY KEY market_intelligence_pkey: PRIMARY KEY (id)
//   CHECK market_intelligence_status_check: CHECK ((status = ANY (ARRAY['draft'::text, 'published'::text])))
// Table: nab_market
//   PRIMARY KEY nab_market_pkey: PRIMARY KEY (id)
// Table: order_items
...
// Table: order_refunds
//   FOREIGN KEY order_refunds_order_id_fkey: FOREIGN KEY (order_id) REFERENCES orders(id) ON DELETE CASCADE
//   PRIMARY KEY order_refunds_pkey: PRIMARY KEY (id)
// Table: order_returns
...
// Table: shipping_configs
//   PRIMARY KEY shipping_configs_pkey: PRIMARY KEY (id)
// Table: shopping_carts
...
//     USING: true
//   Policy "Allow public read access for ai_agent_settings" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "admin_all_ai_agent_settings" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
// Table: ai_providers
//   Policy "admin_all_ai_providers" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
//   Policy "allow_public_read_ai_providers" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: ai_rate_limits
//   Policy "Apenas o sistema pode gerenciar rate limits" (ALL, PERMISSIVE) roles={service_role}
//     USING: true
//     WITH CHECK: true
//   Policy "Sistema gerencia rate limits" (ALL, PERMISSIVE) roles={service_role}
//     USING: true
//     WITH CHECK: true
// Table: ai_settings
//   Policy "Allow public read access" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "Allow public read access for ai_settings" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "Permitir escrita para admins" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true
//   Policy "Permitir leitura pública" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: app_settings
...
//     USING: false
// Table: avpro_keywords
//   Policy "avpro_keywords_admin_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
//   Policy "avpro_keywords_service_role_all" (ALL, PERMISSIVE) roles={public}
//     USING: (auth.role() = 'service_role'::text)
//     WITH CHECK: (auth.role() = 'service_role'::text)
// Table: cache_settings
//   Policy "cache_settings_public_read" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "cache_settings_service_delete" (DELETE, PERMISSIVE) roles={public}
//     USING: (auth.role() = 'service_role'::text)
//   Policy "cache_settings_service_insert" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: (auth.role() = 'service_role'::text)
//   Policy "cache_settings_service_update" (UPDATE, PERMISSIVE) roles={public}
//     USING: (auth.role() = 'service_role'::text)
//     WITH CHECK: (auth.role() = 'service_role'::text)
// Table: cart_items
...
//     USING: true
//     WITH CHECK: true
//   Policy "categories_select_public" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: chat_messages
//   Policy "Enable insert for all users" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: true
//   Policy "Enable select for users based on user_id" (SELECT, PERMISSIVE) roles={public}
//     USING: ((auth.uid() = user_id) OR (user_id IS NULL))
// Table: company_info
//   Policy "company_info_select_public" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "company_info_write_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
// Table: conversation_history
...
//   Policy "Enable INSERT for authenticated users" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "admin_all_customers" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
//   Policy "user_read_own_customer" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (auth.uid() = user_id)
//   Policy "user_update_own_customer" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (auth.uid() = user_id)
//     WITH CHECK: (auth.uid() = user_id)
// Table: discount_coupons
...
//     USING: true
// Table: discount_rule_categories
//   Policy "discount_cats_all_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
//   Policy "discount_cats_read_public" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: discount_rule_customers
//   Policy "discount_custs_all_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
//   Policy "discount_custs_read_public" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: discount_rule_exclusions
//   Policy "discount_excl_all_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
//   Policy "discount_excl_read_public" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: discount_rule_manufacturers
//   Policy "discount_mfg_all_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
//   Policy "discount_mfg_read_public" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: discount_rule_products
//   Policy "discount_prods_all_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
//   Policy "discount_prods_read_public" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: discount_rules
...
// Table: discounts
//   Policy "Allow public read on discounts" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "Enable read access for all users" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "discounts_select_public" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "discounts_write_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
// Table: exchange_rate
//   Policy "exchange_rate_select_public_new" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "exchange_rate_write_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
// Table: favorites
...
//     WITH CHECK: true
// Table: market_intelligence
//   Policy "Allow admin full access" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "Allow public read for published intelligence" (SELECT, PERMISSIVE) roles={public}
//     USING: (status = 'published'::text)
//   Policy "Enable all for authenticated admin users" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true
//   Policy "Enable read access for all users" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "Public Access" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "allow_public_read_market_intelligence" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: nab_market
//   Policy "Public Access" (SELECT, PERMISSIVE) roles={public}
//     USING: true
...
// Table: order_refunds
//   Policy "Admins can manage order refunds" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
// Table: order_returns
...
//     WITH CHECK: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
//   Policy "user_read_own_orders" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
//   Policy "user_update_own_orders" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
//     WITH CHECK: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
// Table: page_visits
...
//     USING: true
//   Policy "Allow public read on price_settings" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: pricing_settings
...
//     USING: true
//   Policy "product_search_cache_write_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
// Table: products
...
//     USING: (EXISTS ( SELECT 1    FROM customers   WHERE ((customers.user_id = auth.uid()) AND ((customers.role)::text = 'admin'::text))))
//   Policy "Allow public read access for products" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "products_select_public_new" (SELECT, PERMISSIVE) roles={public}
//     USING: true
...
//     USING: (EXISTS ( SELECT 1    FROM customers   WHERE ((customers.user_id = auth.uid()) AND ((customers.role)::text = 'admin'::text))))
// Table: settings
//   Policy "settings_select_public_new" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "settings_write_admin" (ALL, PERMISSIVE) roles={authenticated}
//     USING: check_is_admin()
//     WITH CHECK: check_is_admin()
// Table: shipping_configs
//   Policy "admin_all_shipping_configs" (ALL, PERMISSIVE) roles={public}
//     USING: (EXISTS ( SELECT 1    FROM customers   WHERE ((customers.user_id = auth.uid()) AND ((customers.role)::text = 'admin'::text))))
//   Policy "public_read_shipping_configs" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: shopping_carts
...
//     WITH CHECK: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
//   Policy "user_delete_own_cart" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
//   Policy "user_read_own_cart" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
//   Policy "user_update_own_cart" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
//     WITH CHECK: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
// Table: user_sessions
...
//   Policy "Enable insert for anon and authenticated" (INSERT, PERMISSIVE) roles={public}
//     WITH CHECK: true
//   Policy "Enable select for users" (SELECT, PERMISSIVE) roles={public}
//     USING: true
//   Policy "Enable update for anon and authenticated" (UPDATE, PERMISSIVE) roles={public}
//     USING: true
//     WITH CHECK: true
...
//   $function$
//
// FUNCTION check_ai_rate_limit(text, integer)
//   CREATE OR REPLACE FUNCTION public.check_ai_rate_limit(user_identifier text, max_requests integer)
//    RETURNS boolean
//    LANGUAGE plpgsql
//    SECURITY DEFINER
...
//       request_count int;
//   BEGIN
//       -- ATOMICIDADE: Bloqueio consultivo baseado no hash do identificador
//       -- Isso serializa as requisições do MESMO usuário, impedindo o bypass por concorrência.
//       PERFORM pg_advisory_xact_lock(hashtext(user_identifier));
//
//       -- Conta requisições no último minuto usando o índice
...
//       INSERT INTO public.ai_rate_limits (identifier) VALUES (user_identifier);
//       RETURN true;
//   END;
//   $function$
//
...
//   $function$
//
// FUNCTION check_legacy_user(text)
...
//   $function$
//
// FUNCTION complete_user_migration(uuid, uuid)
...
//   $function$
//
// FUNCTION execute_ai_search(text)
...
//       search_words text[];
//   BEGIN
//       -- Limpeza de termos genéricos que não definem o modelo
//       search_term := regexp_replace(lower(search_term), '\b(camera|câmera|equipamento|modelo|aparelho)\b', '', 'g');
//       search_words := string_to_array(trim(regexp_replace(search_term, '[,.!]', '', 'g')), ' ');
//
...
//                       p.stock,
//
//                       -- CÁLCULO DE RELEVÂNCIA SNIPER
//                       (
//                           -- Peso 100: Match exato do modelo (segunda ou terceira palavra da busca)
//                           CASE WHEN p.name ILIKE '%' || search_words[array_upper(search_words, 1)] || '%'
//                               THEN 100 ELSE 0
//                           END +
//
//                           -- Peso 50: Match no SKU
//                           CASE WHEN p.sku ILIKE '%' || search_words[1] || '%'
//                               THEN 50 ELSE 0
//                           END +
//
//                           -- Peso 10: Para cada palavra da busca encontrada no nome
//                           (SELECT count(*) * 10
...
//                   )
//
//                   ORDER BY relevance_score DESC, p.price_usd DESC
//                   LIMIT 20
//               ) p_row
//           ),
//           'intel', '[]'::jsonb,
...
//   $function$
//
// FUNCTION execute_ai_search_v2(text)
...
//   $function$
//
// FUNCTION mark_migration_started(text)
...
//   $function$
//
// FUNCTION products_search_update()
...
//
//       setweight(
//         to_tsvector('simple', coalesce(NEW.sku, '')),
//         'A'
//       )
//
//       ||
...
//
//       setweight(
//         to_tsvector('simple', coalesce(NEW.technical_info, '')),
//         'B'
//       );
//
//     RETURN NEW;
...
//   $function$
//
// FUNCTION search_products_v2(text)
...
//    RETURNS TABLE(id uuid, name text, description text, sku text, manufacturer text, category text, price_usd numeric, price_nationalized_sales numeric, price_nationalized_currency text, final_rank numeric)
//    LANGUAGE plpgsql
//   AS $function$
//   DECLARE
//     clean_term text;
...
//   $function$
//
// FUNCTION sync_current_user_profile()
...
// Table: ai_rate_limits
//   CREATE INDEX idx_ai_rate_limits_identifier_created ON public.ai_rate_limits USING btree (identifier, created_at DESC)
// Table: app_settings
...
// Table: avpro_keywords
//   CREATE INDEX idx_avpro_keywords_category ON public.avpro_keywords USING btree (category)
//   CREATE INDEX idx_avpro_keywords_weight ON public.avpro_keywords USING btree (weight)
// Table: cart_items
...
//   CREATE UNIQUE INDEX idx_customers_unique_email ON public.customers USING btree (email) WHERE (email IS NOT NULL)
...
// Table: market_intelligence
//   CREATE INDEX idx_market_intelligence_manufacturer ON public.market_intelligence USING btree (manufacturer_id)
// Table: orders
