...
//       search_words text[];
//   BEGIN
//       -- Limpeza e preparação dos termos: remove vírgulas e quebra em palavras
//       search_words := string_to_array(regexp_replace(lower(search_term), '[,]', '', 'g'), ' ');
//   
//       SELECT jsonb_build_object(
//           'stock', (
//               SELECT jsonb_agg(p) FROM (
//                   SELECT 
//                       id, name, sku, price_usd, manufacturer_name, ncm, description, technical_info, stock
//                   FROM products
//                   WHERE (
//                       -- Busca flexível: verifica se QUALQUER palavra da busca bate no nome ou SKU
//                       EXISTS (
//                           SELECT 1 FROM unnest(search_words) as word 
//                           WHERE length(word) > 2 
//                           AND (name ILIKE '%' || word || '%' OR sku ILIKE '%' || word || '%')
//                       )
...
//                   ORDER BY 
//                       -- TIER 1: Relevância de Identidade (Nome/SKU)
//                       CASE 
//                           WHEN sku ILIKE '%' || search_term || '%' THEN 1
//                           WHEN name ILIKE '%' || search_term || '%' THEN 2
//                           ELSE 3 
//                       END ASC,
//                       -- TIER 2: Itens sem preço (0 ou NULL) vão para o FIM da lista (Penalização)
//                       CASE WHEN price_usd = 0 OR price_usd IS NULL THEN 1 ELSE 0 END ASC,
//                       -- TIER 3: O MAIS CARO PRIMEIRO (Garante que a Câmera apareça antes do Cabo)
//                       price_usd DESC 
//                   LIMIT 30
//               ) p
//           ),
