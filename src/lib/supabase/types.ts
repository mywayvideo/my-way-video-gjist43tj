...
//                   SELECT 
//                       p.id, p.name, p.sku, p.price_usd, m.name as manufacturer_name, p.ncm, p.description, p.technical_info, p.stock,
//                       -- CÁLCULO DE RELEVÂNCIA SNIPER
//                       (
//                           -- Peso 100: Match exato do modelo (segunda ou terceira palavra da busca)
//                           CASE WHEN p.name ILIKE '%' || search_words[array_upper(search_words, 1)] || '%' THEN 100 ELSE 0 END +
//                           -- Peso 50: Match no SKU
//                           CASE WHEN p.sku ILIKE '%' || search_words[1] || '%' THEN 50 ELSE 0 END +
//                           -- Peso 10: Para cada palavra da busca encontrada no nome
//                           (SELECT count(*) * 10 FROM unnest(search_words) w WHERE p.name ILIKE '%' || w || '%')
//                       ) as relevance_score
//                   FROM products p
...
//                           WHERE length(word) > 2 
//                           AND (p.name ILIKE '%' || word || '%' OR p.sku ILIKE '%' || word || '%')
//                       )
//                   )
