...
//     USING: (check_is_admin() OR ((auth.jwt() ->> 'role'::text) = 'admin'::text) OR (((auth.jwt() -> 'user_metadata'::text) ->> 'role'::text) = 'admin'::text) OR (((auth.jwt() -> 'app_metadata'::text) ->> 'role'::text) = 'admin'::text) OR (auth.uid() = user_id))
...
// Table: discounts
//   Policy "Admins can do everything" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((( SELECT customers.role    FROM customers   WHERE (customers.id = auth.uid())))::text = 'admin'::text)
//     WITH CHECK: ((( SELECT customers.role    FROM customers   WHERE (customers.id = auth.uid())))::text = 'admin'::text)
...
//     USING: true
//   Policy "products_select_public" (SELECT, PERMISSIVE) roles={public}
//     USING: true
// Table: sales_metrics
...
//     WITH CHECK: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
//   Policy "Customers can delete own cart" (DELETE, PERMISSIVE) roles={public}
//     USING: (customer_id IN ( SELECT customers.id    FROM customers   WHERE (customers.user_id = auth.uid())))
